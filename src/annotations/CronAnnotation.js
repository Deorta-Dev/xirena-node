...
class CronAnnotation extends AbstractAnnotation {
    constructor() {
        super();
        this._timezone = '';
        this._interval = '';
...
module.exports = {
    'Cron': function (interval = "*", timezone = 'Etc/Greenwich') {
        if (!timezoneList.includes(timezone)) {
            let tz = 'Etc/Greenwich';
            for (let i in timezoneList)
                if (timezoneList[i].includes(timezone)) {
                    tz = timezoneList[i];
                    break;
                }
            timezone = tz;
        }
        while (interval.split(' ').length < 6) {
            interval += ' *';
        }
        let annotation = new CronAnnotation();
        annotation.interval = interval;
        annotation.timezone = timezone;
        return annotation;
    }
